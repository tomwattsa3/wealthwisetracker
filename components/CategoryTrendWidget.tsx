...
const CategoryTrendWidget: React.FC<CategoryTrendWidgetProps> = ({
  ...
  if (!category) {
    return (
      <div className="bg-white dark:bg-neutral-800 rounded-[10px] border border-slate-200 dark:border-neutral-600 shadow-sm flex flex-col h-[420px] items-center justify-center p-6 text-center">
         <p className="text-slate-400 dark:text-neutral-500 text-sm font-medium">Category not found</p>
      </div>
    );
  }
  ...
  if (variant === 'expense-sheet') {
    ...
    return (
      <div className="bg-white dark:bg-neutral-800 rounded-2xl shadow-sm border border-slate-100 dark:border-neutral-700 flex flex-col h-[420px] overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                ...
                {emojiPickerOpen && (
                  <div className="absolute top-8 left-0 z-50 bg-white dark:bg-neutral-800 rounded-xl shadow-lg border border-slate-200 dark:border-neutral-600 p-2 w-[220px]">
                    <div className="grid grid-cols-6 gap-1">
                      {EMOJI_OPTIONS.map(e => (
                        <button
                          ...
                          className={`w-8 h-8 flex items-center justify-center rounded-lg text-base hover:bg-slate-100 dark:hover:bg-neutral-700 transition-colors ${e === emoji ? 'bg-slate-100 dark:bg-neutral-700 ring-1 ring-slate-300' : ''}`}
                        >
                          {e}
                        </button>
                      ))}
              ...
              <div className="relative">
                <span className="text-sm font-bold text-slate-900 dark:text-neutral-200">{category.name}</span>
                <select
                  value={categoryId}
            ...
            <div className="text-right">
              <span className="text-sm font-bold text-slate-900 dark:text-neutral-200">{formatCurrency(totalAmount)}</span>
              <p className="text-[10px] font-medium text-slate-400 dark:text-neutral-500">{formatAlt(totalAmountAlt)}</p>
            </div>
          </div>
          <div className="flex items-center gap-1.5 mt-1">
            ...
            {category.subcategories.length > 0 && (
              <select
                ...
                className="bg-transparent text-[10px] font-medium text-slate-400 dark:text-neutral-500 outline-none cursor-pointer hover:text-slate-600 transition-colors ml-auto"
              >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {groupedTransactions.length > 0 ? (
              ...
              {groupedTransactions.map((t, idx) => (
                <div key={t.id} className={`grid grid-cols-[1fr_32px_80px] items-center border-b border-dashed border-slate-200/80 last:border-b-0 ${idx % 2 === 1 ? 'bg-slate-50/60 dark:bg-neutral-700/60' : 'bg-white dark:bg-neutral-800'}`}>
                  ...
                  <div className="px-3 py-3 text-right">
                    <span className="text-[11px] font-semibold text-slate-800 dark:text-neutral-300">{formatCurrency(t.amount)}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-2 py-8">
              ...
              <p className="text-xs text-slate-400 dark:text-neutral-500">No transactions</p>
            </div>
          )}
        </div>
      </div>
  ...
  // --- Default variant ---
  return (
    <div className="bg-white dark:bg-neutral-800 rounded-xl border border-slate-200 dark:border-neutral-600 flex flex-col h-[520px] overflow-hidden">

      {/* Mercury Style Header */}
      <div className="px-4 py-3 border-b border-slate-100 dark:border-neutral-700 flex justify-between items-center">
          <div className="flex items-center gap-2">
             ...
             <div className="relative">
                  <div className="flex items-center gap-1.5 cursor-pointer">
                    <span className="text-xs font-medium text-slate-400 dark:text-neutral-500 uppercase tracking-wide">{category.name}</span>
          ...
          {/* Total Amount */}
          <div className="text-right">
            <span className="text-sm font-semibold text-slate-900 dark:text-neutral-200 font-mono">
      ...
      {/* Subfilter */}
      <div className="px-4 py-2 border-b border-slate-50 dark:border-neutral-700 flex items-center justify-between">
            <span className="text-[10px] font-medium text-slate-400 dark:text-neutral-500 uppercase tracking-wide">Transactions</span>

             <select
                 value={subFilter}
                 onChange={(e) => setSubFilter(e.target.value)}
                 className="bg-transparent text-[10px] font-medium text-slate-500 dark:text-neutral-500 outline-none cursor-pointer hover:text-slate-700 dark:hover:text-neutral-300 transition-colors"
             >
                 <option value="all">All</option>
                 {category.subcategories.map(s => (
                 <option key={s} value={s}>{s}</option>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {groupedTransactions.length > 0 ? (
          <div className="md:block">
            ...
            <div className="md:hidden grid grid-cols-[1fr_auto_auto] bg-slate-50/80 dark:bg-neutral-700/80 border-b border-slate-200 dark:border-neutral-600">
            ...
            <div className="hidden md:grid grid-cols-[1fr_24px_80px] bg-slate-100 dark:bg-neutral-700 border-b border-dashed border-slate-200/80 sticky top-0 z-10">
            ...
            {groupedTransactions.map((t, idx) => (
              <div key={t.id}>
                ...
                <div className="md:hidden grid grid-cols-[1fr_auto_auto] items-center bg-white dark:bg-neutral-800 border-b border-dashed border-slate-200/80 last:border-b-0">
                  <div className="px-3 py-3 border-r border-dashed border-slate-200/80">
                    <span className="text-sm font-medium text-slate-700 dark:text-neutral-400 block truncate" title={t.description}>{t.description || "Unknown"}</span>
                  ...
                  <div className="px-3 py-3 text-center border-r border-dashed border-slate-200/80 w-12">
                    <span className="text-sm text-slate-500 dark:text-neutral-500">{t.count > 1 ? t.count : '-'}</span>
                  </div>
                  <div className="px-3 py-3 text-right w-24">
                    <span className="text-sm font-semibold text-slate-800 dark:text-neutral-300">{formatCurrency(t.amount)}</span>
                  </div>
                </div>

                {/* Desktop Spreadsheet Row */}
                <div className={`hidden md:grid grid-cols-[1fr_24px_80px] items-center bg-white dark:bg-neutral-800 border-b border-dashed border-slate-200/80 last:border-b-0`}>
                  <div className="px-2.5 py-2 border-r border-dashed border-slate-200/80 min-w-0 flex items-center justify-between gap-1">
                    <span className="text-[11px] font-medium text-slate-700 dark:text-neutral-400 truncate" title={t.description}>{t.description || "Unknown"}</span>
                    <span className="px-1 py-0.5 bg-slate-100 dark:bg-neutral-700 rounded text-[7px] text-slate-600 dark:text-neutral-500 shrink-0 leading-none">{t.subcategoryName}</span>
                  </div>
