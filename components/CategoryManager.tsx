...
const CategoryManager: React.FC<CategoryManagerProps> = ({
    ...
    <button
      ...
      className={`w-full flex items-center justify-between p-2 sm:p-4 rounded-lg sm:rounded-xl transition-all border group ${
        selectedCategoryId === cat.id
          ...
          : 'bg-white dark:bg-neutral-800 border-transparent hover:bg-slate-50 dark:hover:bg-neutral-700 hover:border-slate-100 dark:hover:border-neutral-700'
      }`}
    >
       <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="text-left">
             ...
             <p className="text-[10px] sm:text-xs text-slate-400 dark:text-neutral-500 font-medium">{cat.subcategories.length} subs</p>
          </div>
       </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 h-[calc(100vh-180px)] sm:h-[calc(100vh-140px)] min-h-[400px] sm:min-h-[600px] gap-2 sm:gap-6 animate-in fade-in px-2 sm:px-0">
      ...
      <div className="flex flex-col bg-white dark:bg-neutral-800 rounded-xl sm:rounded-2xl border border-slate-200 dark:border-neutral-600 shadow-sm overflow-hidden h-full">

        {/* Header */}
        <div className="p-2 sm:p-3 border-b border-slate-100 dark:border-neutral-700 bg-slate-50/50 dark:bg-neutral-700/50 space-y-2 sm:space-y-3">
           <div className="flex items-center justify-between">
              <h2 className="font-bold text-slate-800 dark:text-neutral-300 flex items-center gap-1.5 sm:gap-2 text-sm sm:text-lg">
           ...
           <div className="relative">
              <Search size={14} className="sm:w-4 sm:h-4 absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-neutral-500" />
              <input
                type="text"
                placeholder="Search..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-7 sm:pl-10 pr-3 sm:pr-4 py-2 sm:py-3 bg-white dark:bg-neutral-700 border border-slate-200 dark:border-neutral-600 rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold dark:text-neutral-300 focus:border-violet-500 focus:ring-2 focus:ring-violet-100 outline-none"
              />
           </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2 sm:p-3 space-y-3 sm:space-y-6">
           ...
           {groupedCategories.income.length > 0 && (
             <div className="space-y-1 sm:space-y-2">
               <h3 className="px-1 sm:px-2 text-[10px] sm:text-xs font-bold text-slate-400 dark:text-neutral-500 uppercase tracking-wider">Income</h3>
           ...
           {groupedCategories.expense.length > 0 && (
             <div className="space-y-1 sm:space-y-2">
               <h3 className="px-1 sm:px-2 text-[10px] sm:text-xs font-bold text-slate-400 dark:text-neutral-500 uppercase tracking-wider">Expenses</h3>
           ...
           {groupedCategories.excluded.length > 0 && (
             <div className="space-y-1 sm:space-y-2">
               <h3 className="px-1 sm:px-2 text-[10px] sm:text-xs font-bold text-slate-400 dark:text-neutral-500 uppercase tracking-wider">Excluded</h3>
               {groupedCategories.excluded.map(renderCategoryRow)}
             </div>
           )}

           {filteredCategories.length === 0 && (
             <div className="p-4 sm:p-8 text-center text-slate-400 dark:text-neutral-500">
      ...
      <div className="flex flex-col bg-white dark:bg-neutral-800 rounded-xl sm:rounded-2xl border border-slate-200 dark:border-neutral-600 shadow-sm overflow-hidden h-full relative">
         {selectedCategory ? (
           <>
              {/* Header */}
              <div className="p-2 sm:p-3 border-b border-slate-100 dark:border-neutral-700 flex justify-between items-start bg-slate-50/30 dark:bg-neutral-700/30">
                 <div>
                    <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
                       <div className="relative">
                         ...
                         {showEmojiPicker && (
                           <div className="absolute top-9 left-0 z-50 bg-white dark:bg-neutral-800 rounded-xl shadow-lg border border-slate-200 dark:border-neutral-600 p-2.5 w-[280px] max-h-[320px] overflow-y-auto custom-scrollbar">
                             <p className="text-[9px] font-semibold text-slate-400 dark:text-neutral-500 uppercase tracking-wider mb-2 px-1">Choose emoji</p>
                             <div className="grid grid-cols-8 gap-1">
                               {EMOJI_OPTIONS.map(e => (
                                 <button
                                   ...
                                   className={`w-7 h-7 flex items-center justify-center rounded-lg text-base hover:bg-slate-100 dark:hover:bg-neutral-700 transition-colors ${e === (getCategoryEmoji ? getCategoryEmoji(selectedCategory.id) : '') ? 'bg-violet-50 ring-1 ring-violet-300' : ''}`}
                                 >
                                   {e}
                                 </button>
                               ))}
                       ...
                       <h2 className="text-sm sm:text-xl font-bold text-slate-900 dark:text-neutral-200">{selectedCategory.name}</h2>
                    </div>
                    <p className="text-slate-500 dark:text-neutral-500 text-[10px] sm:text-xs font-medium ml-8 sm:ml-9">Subcategories</p>
                 </div>
              ...
              <div className="p-2 sm:p-4 border-t border-slate-100 dark:border-neutral-700 bg-slate-50 dark:bg-neutral-700">
                 <form onSubmit={handleAddSubcategory} className="flex gap-1.5 sm:gap-2">
                    <input
                      ...
                      className="flex-1 px-2 sm:px-3 py-2 sm:py-2.5 bg-white dark:bg-neutral-700 border border-slate-200 dark:border-neutral-600 rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold dark:text-neutral-300 focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition-all"
                    />
                    <button
                      type="submit"
                      disabled={!newSubcatName.trim()}
                      className="px-3 sm:px-5 py-2 sm:py-2.5 bg-slate-900 dark:bg-neutral-600 text-white font-bold rounded-lg sm:rounded-xl hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-slate-200 flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm"
                    >
           ...
           <div className="flex-1 flex flex-col items-center justify-center text-slate-300 dark:text-neutral-600">
           ...
           <div className="relative w-full max-w-sm sm:max-w-lg bg-white dark:bg-neutral-800 rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
              ...
              <form onSubmit={handleCreateCategory} className="p-3 sm:p-6 space-y-4 sm:space-y-6">
                 <div className="space-y-3 sm:space-y-4">
                    <div>
                       <label className="block text-[10px] sm:text-xs font-bold text-slate-500 dark:text-neutral-500 uppercase mb-1 sm:mb-1.5">Name</label>
                       <input
                         autoFocus
                         type="text"
                         value={newCatName}
                         ...
                         className="w-full px-3 sm:px-4 py-2 sm:py-3 bg-white dark:bg-neutral-700 border border-slate-200 dark:border-neutral-600 rounded-lg sm:rounded-xl font-bold text-sm sm:text-base text-slate-800 dark:text-neutral-300 focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition-all"
                       />
                    </div>

                    <div>
                       <label className="block text-[10px] sm:text-xs font-bold text-slate-500 dark:text-neutral-500 uppercase mb-1 sm:mb-1.5">Type</label>
                       <div className="grid grid-cols-2 gap-2 sm:gap-3">
                          <button
                            type="button"
                            onClick={() => setNewCatType('EXPENSE')}
                            className={`py-2 sm:py-3 rounded-lg sm:rounded-xl text-xs sm:text-sm font-bold border transition-all ${newCatType === 'EXPENSE' ? 'bg-rose-50 border-rose-200 text-rose-700 ring-2 ring-rose-500/20' : 'bg-white dark:bg-neutral-700 border-slate-200 dark:border-neutral-600 text-slate-500 dark:text-neutral-500 hover:bg-slate-50 dark:hover:bg-neutral-700'}`}
                          >
                             Expense
                          </button>
                          <button
                            ...
                            className={`py-2 sm:py-3 rounded-lg sm:rounded-xl text-xs sm:text-sm font-bold border transition-all ${newCatType === 'INCOME' ? 'bg-emerald-50 border-emerald-200 text-emerald-700 ring-2 ring-emerald-500/20' : 'bg-white dark:bg-neutral-700 border-slate-200 dark:border-neutral-600 text-slate-500 dark:text-neutral-500 hover:bg-slate-50 dark:hover:bg-neutral-700'}`}
                          >
                             Income
                          </button>
                       </div>
                    ...
                    <div>
                       <label className="block text-[10px] sm:text-xs font-bold text-slate-500 dark:text-neutral-500 uppercase mb-1.5 sm:mb-2">Color</label>
           ...
           <div className="relative w-full max-w-sm sm:max-w-lg bg-white dark:bg-neutral-800 rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
              ...
              <form onSubmit={handleSaveEdit} className="p-3 sm:p-6 space-y-4 sm:space-y-6">
                 <div className="space-y-3 sm:space-y-4">
                    ...
                    <div>
                       <label className="block text-[10px] sm:text-xs font-bold text-slate-500 dark:text-neutral-500 uppercase mb-1 sm:mb-1.5">Type</label>
                       <div className="py-2 sm:py-3 px-3 sm:px-4 bg-slate-50 dark:bg-neutral-700 rounded-lg sm:rounded-xl text-xs sm:text-sm font-bold text-slate-500 dark:text-neutral-500 border border-slate-200 dark:border-neutral-600">
                          {selectedCategory.type === 'INCOME' ? 'Income' : 'Expense'}
                          <span className="text-slate-400 dark:text-neutral-500 font-medium ml-2">(cannot be changed)</span>
                       </div>
                    </div>
